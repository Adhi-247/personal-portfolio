...
const Certificates = () => {
  ...
  // Auto-rotate images for certificates with multiple images
  useEffect(() => {
    const interval = setInterval(() => {
      setImageIndices(prev => {
        const newIndices = { ...prev };
        experiences.forEach((cert) => {
          if (cert.images && cert.images.length > 1) {
            const currentIndex = prev[cert.id] || 0;
            newIndices[cert.id] = (currentIndex + 1) % cert.images.length;
          }
        });
        return newIndices;
      });
          ...
          >
            ...
            <div className="relative p-8 space-y-5 z-10">
              ...
              >
                ...
                >
                  <motion.img
                    src={cert.images ? cert.images[imageIndices[cert.id] || 0] : cert.img}
                    alt={cert.company}
                    className="w-32 h-32 object-contain rounded-xl"
                    key={imageIndices[cert.id] || 0}
                  ...
                  {cert.images && cert.images.length > 1 && (
                    <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex gap-1">
                      {cert.images.map((_, imgIdx) => (
                        <div
                          key={imgIdx}
                          className={`w-1.5 h-1.5 rounded-full transition-all ${
                            (imageIndices[cert.id] || 0) === imgIdx
                              ? 'bg-purple-500 w-3'
                              : 'bg-gray-400'
                          }`}
                        />
